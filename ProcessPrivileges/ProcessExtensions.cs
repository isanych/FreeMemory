...
namespace ProcessPrivileges
{
    ...
    public static class ProcessExtensions
    {
        ...
        public static void EnablePrivilege(this AccessTokenHandle accessTokenHandle, params Privilege[] privilege)
        {
            Privileges.EnablePrivilege(accessTokenHandle, privilege);
        }

        /// <summary>Enable Privileges</summary>
        [MethodImpl(MethodImplOptions.Synchronized), PermissionSet(SecurityAction.LinkDemand, Name = "FullTrust")]
        public static void EnablePrivilege(this Process process, params Privilege[] privilege)
        {
            using (var handle = new AccessTokenHandle(process, TokenAccessRights.AdjustPrivileges | TokenAccessRights.Query))
            {
                handle.EnablePrivilege(privilege);
            }
        }
        ...
        {
            using (var handle = new AccessTokenHandle(process, TokenAccessRights.Query))
            {
