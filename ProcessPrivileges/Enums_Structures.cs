...
namespace ProcessPrivileges
{
	...
	public static class StaticInfo
	{
		...
		public const uint
			TokenPrivilegeArray_Offset = sizeof(Int32);
		/// <summary>Each array element is this size</summary>
		public static uint
			LuidAndAttributes_Size;
...
#pragma warning restore 1591
	...
	public class TokenPrivilegeArray : IDisposable
	{
		...
		public virtual UInt32 PrivilegeCount
		{
			get 
			{
			...
			set 
			{
				if (_internalPtr == null)
					throw new ObjectDisposedException("TokenPrivilegeArray");
				Dispose(false);
				_internalPtr = Marshal.AllocHGlobal((int)(StaticInfo.LuidAndAttributes_Size * value + sizeof(UInt32)));
		...
		public virtual LuidAndAttributes[] Privileges
		{
			set 
			{
				...
				try
				{
					this.PrivilegeCount = (uint)value.Length;
					uint
						size = StaticInfo.LuidAndAttributes_Size * (uint)value.Length;

					IntPtr
						dest = new IntPtr(((IntPtr)_internalPtr).ToInt64() + StaticInfo.TokenPrivilegeArray_Offset);
...
//					NativeMethods.MemCopy(dest.ToPointer(), src.ToPointer(), new UIntPtr(size));
		...
		public unsafe LuidAndAttributes[] CopyPrivileges()
		{
			...
			LuidAndAttributes[]
				result = new LuidAndAttributes[_currentCount];

			uint
				size = _currentCount * StaticInfo.LuidAndAttributes_Size;

			IntPtr
				src = new IntPtr(((IntPtr)_internalPtr).ToInt64() + StaticInfo.TokenPrivilegeArray_Offset);
		...
		public LuidAndAttributes this[uint Index]
		{
			set
			{
				...
				try
				{
					...
					Marshal.StructureToPtr(value, src, false);
					IntPtr
						dest = new IntPtr(((IntPtr)_internalPtr).ToInt64() + StaticInfo.TokenPrivilegeArray_Offset + Index * StaticInfo.LuidAndAttributes_Size);
		...
		public virtual uint CurrentSize
		{ get { return StaticInfo.LuidAndAttributes_Size * _currentCount; } }

		/// <summary>Instansiate</summary>
		public TokenPrivilegeArray()
			: this(1)
		{ }

		/// <summary>Instansiate with known array size</summary>
		public TokenPrivilegeArray(UInt32 Count)
		{
			_internalPtr = Marshal.AllocHGlobal((int)(StaticInfo.LuidAndAttributes_Size * Count + sizeof(UInt32)));
